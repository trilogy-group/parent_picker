...
export function HomeContent({ altUI }: { altUI: boolean }) {
  const [panelExpanded, setPanelExpanded] = useState(false);
  const { locations, citySummaries, zoomLevel, loadCitySummaries, setReferencePoint, setIsAdmin, releasedFilter, showUnscored, viewAsParent, setShowAltUI } = useVotesStore();
  const { isAdmin } = useAuth();

  // Sync altUI prop into store so MapView can read it
  useEffect(() => {
    setShowAltUI(altUI);
  }, [altUI, setShowAltUI]);
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden">
      ...
      <div className={altUI ? "absolute inset-0 hidden lg:block" : "absolute inset-0"}>
        <Map />
      </div>
